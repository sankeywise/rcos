...
function getProjectDropdownLabel(project: ProjectRow) {
  const name =
    project.project_name && project.project_name.trim().length > 0
      ? project.project_name.trim()
      : `Unnamed Project ${project.id}`;

  const sponsor =
    project.sponsor && project.sponsor.trim().length > 0
  ...
  async function updateIntakeReview(formData: FormData) {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        review_date: String(formData.get("review_date") || "").trim() || null,
        final_determination:
          String(formData.get("final_determination") || "").trim() || null,
        determination_notes:
          String(formData.get("determination_notes") || "").trim() || null,
        updated_at: new Date().toISOString(),
      })
    ...
    revalidatePath("/project-intake");
    revalidatePath(`/project-intake/${reviewId}`);
    revalidatePath("/");
    revalidatePath("/audit-mode");
    revalidatePath("/project-compliance");
  }
                  ...
                  >
                    ...
                    {projects.map((project) => (
                      <option key={project.id} value={project.id}>
                        {getProjectDropdownLabel(project)}
                      </option>
                    ))}
                  </select>
                </div>
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                        <div>
                          ...
                          <select
                            name="review_status"
                            defaultValue={review.review_status || "Draft"}
                        ...
                        <div>
                          ...
                          <input
                            name="review_owner"
                            defaultValue={review.review_owner || ""}
                            className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
                            placeholder="ECO, ISO, FSO, Compliance"
                          />
                        </div>
                        ...
                        <div>
                          ...
                          <select
                            name="final_determination"
                            defaultValue={review.final_determination || ""}
                            className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
                          >
                        ...
                        <div className="md:col-span-2">
                          ...
                          <textarea
                            name="determination_notes"
                            rows={3}
                            defaultValue={review.determination_notes || ""}
                            className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
                            placeholder="Final notes, routing comments, or review decision. Avoid sensitive technical details."
                          />
                        </div>
                      </div>
